...
if ( !class_exists( 'WTARS_Shipped_Cart_Fees_Engine_Min' ) ) {

    class WTARS_Shipped_Cart_Fees_Engine_Min {
        ...
        public function __construct() {
            ...
            add_filter( 'wtars_shipped/cart-fees/calculated-cost', array( $this, 'get_calculated_cost' ), 10, 3 );
        }

        public function get_calculated_cost( $cost, $fee_id, $data ) {

            $min_args = $this->get_min_args( $fee_id, $data );
        ...
        private function get_min_args( $fee_id, $data ) {
            ...
            $min_options = $this->get_min_options( $fee_id, $instance_id );
        ...
        private function get_min_options( $fee_id, $instance_id ) {

            $options = $this->get_options( $fee_id, $instance_id );
        ...
        private function get_options( $fee_id, $instance_id ) {

            foreach ( WTARS_Shipped::get_option( 'cart_fees', $instance_id, array() ) as $fee_option ) {

                if ( $fee_id == $fee_option[ 'fee_id' ] ) {
    ...
    new WTARS_Shipped_Cart_Fees_Engine_Min();
}
